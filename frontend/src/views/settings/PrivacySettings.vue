...
get("/api/user/privacy",(data)=>{
  saving.value = false
  boxForm.phone = data.phone
  boxForm.email = data.email
  boxForm.wx = data.wx
  boxForm.qq = data.qq
  boxForm.gender = data.gender
})
...
<template>
  <div style="max-width: 600px;margin: auto">
    <Card style="margin: 20px 0" title="隐私设置" desc="在这里设置哪些内容可以被其他人看到，请各位小伙伴注重自己的隐私" :icon="Setting" v-loading="saving">
      <div class="checkBox" >
        <el-checkbox @change="savePrivacy('phone',boxForm.phone)"
            v-model="boxForm.phone">公开展示我的手机号</el-checkbox>
        <el-checkbox @change="savePrivacy('email',boxForm.email)"
            v-model="boxForm.email">公开展示我的电子邮箱地址</el-checkbox>
        <el-checkbox @change="savePrivacy('wx',boxForm.wx)"
            v-model="boxForm.wx">公开展示我的微信号</el-checkbox>
        <el-checkbox @change="savePrivacy('qq',boxForm.qq)"
            v-model="boxForm.qq">公开展示我的QQ号</el-checkbox>
        <el-checkbox @change="savePrivacy('gender',boxForm.gender)"
            v-model="boxForm.gender">公开展示我的性别</el-checkbox>
      </div>
    </Card>
